...
import com.realting.world.content.dialogue.impl.ExplorerJack;
		...
		TaskManager.submit(new Task(5, player, false) {
			...
			protected void execute() {
				try {
					...
					for(Map.Entry<Integer, ItemDefinition> def : ItemDefinition.getEntries()) {
						if(def != null) {
							if(def.getValue().getName().contains(syntaxCopy) && itemId == -1) {
								itemId = def.getValue().getId();
							}
							if(def.getValue().getName().equalsIgnoreCase(syntaxCopy)) {
								itemId = def.getValue().getId();
								break;
							}
						}
