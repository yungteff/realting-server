...
import com.realting.model.input.impl.EnterSyntaxToSearchDropsFor;
	...
	public static List<Integer> getList(String search) {
		...
		for (Map.Entry<Integer, NpcDefinition> definition : NpcDefinition.getEntries()) {
			...
			if (definition.getValue().getName() == null
					|| definition.getValue().getName().equalsIgnoreCase("null")
					|| definition.getValue().getName().equalsIgnoreCase("")
			...
			if (!definition.getValue().getName().toLowerCase().contains(search)) {
				//System.out.println("Skipping "+NpcDefinition.forId(i).getName().toLowerCase());
				continue;
			}
			
			for (int l = 0; l < list.size(); l++) {
				if (NpcDefinition.forId(list.get(l)).getName().equalsIgnoreCase(definition.getValue().getName())) {
					dupe = true;
			...
			if (!dupe) {
				...
				list.add(definition.getValue().getId());
			}	
