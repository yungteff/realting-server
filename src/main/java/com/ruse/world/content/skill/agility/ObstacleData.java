			...
			TaskManager.submit(new Task(1, player, false) {
				...
				public void execute() {
					...
					if(!success) {
						...
						player.dealDamage(new Hit(null, Misc.getRandom(50), Hitmask.RED, CombatIcon.NONE));
				...
				public void execute() {
					...
					if(tick == 2) {
						if(fail) {
							...
							player.dealDamage(new Hit(null, Misc.getRandom(60), Hitmask.RED, CombatIcon.NONE));
							Agility.addExperience(player, 40);
							player.getPacketSender().sendMessage("You failed to swing your way across.");
							stop();
							return;
