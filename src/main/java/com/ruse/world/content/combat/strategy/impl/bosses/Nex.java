					...
					TaskManager.submit(new Task(1, NEX, false) {
						...
						public void execute()
						{
							...
							if(ticks == 5)
							{
								...
								{
									p_.dealDamage(new Hit(NEX, 100 + Misc.getRandom(100), Hitmask.RED, CombatIcon.NONE));
						...
						public void execute()
						{
							if(ticks == 10)
								...
								if(dist < 3)
								{
									...
									p_.dealDamage(new Hit(NEX, 10, Hitmask.RED, CombatIcon.NONE));
									setShadow(p_, 20);
								}
								if(dist >= 3 && dist < 5)
									setShadow(p_, 40);
			...
			if(rnd >= 0 && rnd <= 3 && !attacks[0])
			{
				...
				TaskManager.submit(new Task(10, NEX, false) {
					...
					public void execute()
					{
						...
						{
							p.dealDamage(new Hit(NEX, 250 + Misc.getRandom(150), Hitmask.RED, CombatIcon.NONE));
						}
						for(int x = origX-1; x < origX+1; x++)
						{
							for(int y = origY-1; y < origY+1; y++)
								...
								if(x == origX-ticks || y == origY-ticks || x == origX+ticks || y == origY+ticks)
								{
									...
									for(Player p_ : Misc.getCombinedPlayerList(p))
									{
										...
										if(p_.getPosition().getX() == x && p_.getPosition().getY() == y)
											p_.dealDamage(new Hit(NEX, 150 + Misc.getRandom(110), Hitmask.RED, CombatIcon.NONE));
									}
								}
							}
						}
		...
		TaskManager.submit(new Task(4) {
			...
			public void execute() {
				for(Player p : World.getPlayers())
				{
					...
					{
						p.dealDamage(new Hit(NEX, 150, Hitmask.RED, CombatIcon.NONE));
					}	
