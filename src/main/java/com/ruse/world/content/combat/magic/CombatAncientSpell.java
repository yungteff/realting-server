...
public abstract class CombatAncientSpell extends CombatSpell {
    ...
    public void finishCast(CharacterEntity cast, CharacterEntity castOn, boolean accurate,
        ...
        if (spellRadius() == 0 || !Locations.inMulti(castOn)) {
            return;
        }

        // We passed the checks, so now we do multiple target stuff.
        Iterator<? extends CharacterEntity> it = null;
        ...
        for (Iterator<? extends CharacterEntity> $it = it; $it.hasNext();) {
            ...
            if(next.isNpc()) {
            	...
            	if(p.getLocation() != Location.WILDERNESS || !Locations.inMulti(p)) {
            		continue;
            	}
            }
            ...
            if (next.getPosition().isWithinDistance(castOn.getPosition(),
                ...
                next.dealDamage(new Hit(cast, calc, Hitmask.RED, CombatIcon.MAGIC));
     ...
     */
    public abstract void spellEffect(CharacterEntity cast, CharacterEntity castOn, int damage);
