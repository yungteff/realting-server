...
public class EnterAmountToRemoveFromBank extends EnterAmount {
	...
	public void handleAmount(Player player, int amount) {
		...
		if(amount <= 0)
			return;
		Item itemObj = player.getBank(tab).getById(item);
		if (itemObj.getAttributes().hasAttributes()) {
			player.getBank(tab).setPlayer(player).switchItem(player.getInventory(), itemObj, player.getBank(tab).getSlot(item), false, true);
		} else {
